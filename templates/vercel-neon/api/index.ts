...
const outer = new Outer({
  ...
  db: {
    dialect: new NeonDialect({ neon: neon(secrets.require("DATABASE_URL")) }),
  ...
  // `access: "private"` — bytes reach the browser only through Outer's `GET /files/:id`
  // (permissions applied there). Token comes from `BLOB_READ_WRITE_TOKEN` env.

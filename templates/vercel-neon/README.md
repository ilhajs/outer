...
- **`scripts/migrate.ts`** — runs `outer.migrator.migrateToLatest()` once, reusing the exact same `Outer` instance exported from `api/index.ts`, and seeds the `ADMIN_EMAIL` admin account when set — that account signs in via email OTP and can manage the instance from [Outer Hub](https://hub.outer.now).

...
Outer uses one permission vocabulary everywhere: on [procedures](/outer/procedure), on [resources](/outer/resource), and on [`.files()`](/outer/files). You declare the guard next to the endpoint instead of writing the check inside the handler.

## Before you begin

Every value except `"public"` needs a session, so call [`.auth()`](/outer/auth) somewhere in the chain. Forget it and `.build()` throws at startup, listing the endpoints that need it.
...
`{name}.live` reuses the `list` permission rather than adding one of its own, so a stream can never be looser than the one-shot read. See [Stream a list with `live`](/outer/resource#stream-a-list-with-live) for how long-lived subscriptions are re-checked.
...
One difference is worth knowing: an `"owner"`-guarded download answers **`404`, not `403`**, so file IDs cannot be probed for existence. See [`.files()`](/outer/files#files-are-private-by-default).
...
- [`.auth()`](/outer/auth) — where the session these checks read comes from
- [`.resource()`](/outer/resource) — the actions you are guarding

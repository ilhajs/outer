...
**`live` (reactive queries)** powers [live queries in Sola](/outer/database#live-queries). You never touch the extension directly; `pglite()` wires it up as a `LiveProvider`.
...
The same persistence question applies to file bytes. [`.files()`](/outer/files) needs an `OuterStorage` on the constructor, and the right backing store is the one your host actually persists.
...
If your frontend is deployed on a different origin than the Outer server, add [`new Outer({ cors: { origins } })`](/outer/auth#cors). Without it, browsers block cross-origin calls to `/rpc/**` and `/api/auth/**`.

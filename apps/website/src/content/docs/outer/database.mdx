...
`user` and `session` need no middleware — see [Read the session](/outer/auth#read-the-session). `storage` is the same store [`.files()`](/outer/files) writes through, so a custom procedure can read and write blobs alongside the built-in file API.
...
`include` loads relations you declared with [`.relation()`](/schema/defining-schema#relations). It runs one extra query per relation and merges the results in JS, Prisma-style.
...
To expose a live query without writing a procedure, use [`live: true` on a resource](/outer/resource#stream-a-list-with-live).
...
- [Realtime](/outer/realtime) — stream application events (`EventPublisher`) instead of reactive row snapshots
- [`.resource()`](/outer/resource) — the same query surface, generated

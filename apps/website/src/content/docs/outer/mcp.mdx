---
title: .mcp()
description: Serve your procedures as MCP tools with .mcp(), opt in per procedure, and authenticate agents with API keys.
sidebar:
  order: 6
previousSlug:
  - /guide/mcp
...
Browsers can authenticate with a cookie. Headless clients use an [API key](/outer/auth#api-keys):
...
- [API keys](/outer/auth#api-keys) — issue tokens for headless clients
- [`.procedure()`](/outer/procedure) — write the procedures you expose

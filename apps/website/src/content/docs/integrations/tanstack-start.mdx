...
The same split applies to [`.files()`](/outer/files): local disk via unstorage when self-hosted, an object store — R2, Vercel Blob, any S3-compatible bucket — when not. Pass either as `new Outer({ storage })`.

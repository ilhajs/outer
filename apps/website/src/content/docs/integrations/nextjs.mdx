...
The same split applies to [`.files()`](/outer/files): self-hosted can keep bytes on local disk via unstorage, while on Vercel they belong in [Vercel Blob](https://vercel.com/docs/vercel-blob). Pass either as `new Outer({ storage })`.

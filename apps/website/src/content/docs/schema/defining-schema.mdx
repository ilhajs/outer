...
The `schema()` builder describes your tables and the links between them. It is also your migration history: every version you register becomes one migration step, which [Migrations](/schema/migrations) covers.
...
Pass `.auth({ apiKeys: true })` to also declare the `apikey` table that [API keys](/outer/auth#api-keys) need.
...
The schema only registers the tables. To serve the bytes, call [`.files()`](/outer/files) on the builder chain.
...
Relations drive `include` in both the [Sola ORM](/outer/database#include) and [resources](/outer/resource#include-relations).
...
- [Database](/outer/database) — query the tables you just defined

...
To avoid restating every table when only one column changed, derive the next version with [`.extend()`](/guide/schema#extend-a-previous-version):
...
- [Schema](/guide/schema) — evolve versions with `.extend()`

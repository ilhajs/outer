...
`list` defaults to 50 rows and rejects a `take` above 100 (`400 BAD_REQUEST`) — pass `listLimit: { default, max }` in `options` to change either bound. This keeps an unqualified `.resource()` call from becoming an unbounded `SELECT *` on a large table; reach for `context.db.query[table].paginate(...)` directly in a custom `.procedure()` when you need real pagination metadata.

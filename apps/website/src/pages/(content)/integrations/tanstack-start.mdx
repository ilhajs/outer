...
On the server, skip the network: Outer lives in the same process, so call it through `outer.handle()` directly. Use the plain-JSON `/rest` surface (enabled by `.openapi()`) and forward the incoming request's headers so permissions see the caller's session:

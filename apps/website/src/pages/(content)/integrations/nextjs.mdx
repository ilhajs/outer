...
Since Outer lives in the same process, Server Components can call it through `outer.handle()` directly — an in-process function call, no network hop. Use the plain-JSON `/rest` surface (enabled by `.openapi()`) and forward the incoming headers so permissions see the caller's session:
...
export default async function PostsPage() {
  const res = await outer.handle(
    new Request("http://internal/rest/post/list", {
      method: "POST",
